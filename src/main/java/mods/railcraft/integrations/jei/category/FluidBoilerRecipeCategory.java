...
public class FluidBoilerRecipeCategory implements IRecipeCategory<FluidBoilerJEIRecipe> {
  ...
  @Override
  public void draw(FluidBoilerJEIRecipe recipe, IRecipeSlotsView recipeSlotsView, PoseStack poseStack,
      double mouseX, double mouseY) {
    this.flame.draw(poseStack, 46, 19);
    this.bar.draw(poseStack, 24, 6);

    var font = Minecraft.getInstance().font;
    var temp = recipe.temperature() + "°C";
    font.draw(poseStack, temp, getBackground().getWidth() - font.width(temp) - 50, 43, RailcraftJeiPlugin.TEXT_COLOR);
  }
  ...
  public void setRecipe(IRecipeLayoutBuilder builder, FluidBoilerJEIRecipe recipe, IFocusGroup focuses) {
    ...
    builder.addSlot(RecipeIngredientRole.INPUT, 73, 4)
        .addIngredient(ForgeTypes.FLUID_STACK, recipe.fuel())
        .setFluidRenderer(10_000, true, 16, 47);
